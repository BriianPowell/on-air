...
Default layout (`LED_COUNT 56`):

| Pixels | Person | Topic suffix |
|--------|--------|--------------|
| 0–27 | Brian | `brian-mac` |
| 28–55 | Lauren | `lauren-win` |
...
| 2+ | on (per zone) | **Flash** on that zone only |

Camera on = zone toggles on/off. Tune `CAMERA_FLASH_MS` and colors in `firmware/esp8266/include/config.h`.
